...
static void draw_text(cv::Mat& img, std::string text, cv::Point org, cv::Scalar color = cv::Scalar(0, 0, 0))
{
	int baseline;
	if (text.length() == 0)return;
	auto size = getTextSize(text, cv::HersheyFonts::FONT_HERSHEY_SIMPLEX, 0.5, 1, &baseline);
	cv::Point tmp_pt = { bound(org.x, 0, src.cols - size.width),
		bound(org.y, size.height + baseline, src.rows - 1 - baseline) };
	putText(src, text, tmp_pt, cv::HersheyFonts::FONT_HERSHEY_SIMPLEX, 0.5, color, 1, cv::LineTypes::LINE_AA);
}
...
#endif // !DEBUG
{
	...
	ori = cv::imread("C:/Users/Administrator/Desktop/testPic1.jpg");
	src = dst = ori.clone();
